...
const VariantManagerEdit = ({
  ...
  useEffect(() => {
    if (
      variantEdit &&
      JSON.stringify(variantEdit) !== JSON.stringify(variants)
    ) {
      setVariants(variantEdit);
    }
  }, [variantEdit]);
  ...
  //Chuyen che do edit save = true => false
  const handleEdit = (id) => {
    setVariants((prev) =>
      prev.map((v) => (v.id === id ? { ...v, save: false } : v))
    );
  };
    ...
    <div className="text-end w-full">
      ...
      {currentItem.map((variant) => (
        <VariantEach
          key={variant?.id}
          id={variant?.id}
          initImage={variant?.images}
          initDiscount={variant?.discount}
          initSize={variant?.sizes}
          initColor={variant?.color}
          initBasePrice={variant?.price}
...
const VariantEach = ({
  ...
  const [sizes, setSizes] = useState(() => {
    const base = Array.from({ length: 10 }, (_, i) => i + 35).reduce(
      (acc, size) => {
        acc[size] = "";
        return acc;
      },
    ...
    return initSize ? { ...base, ...initSize } : base;
  });

  const [basePrice, setBasePrice] = useState(initBasePrice || 0);
  ...
  // State quản lý ảnh
  const [images, setImages] = useState(initImage || Array(5).fill(null));
      ...
      <div className="button-group w-full flex justify-end gap-2">
        ...
        {!save ? (
          ...
          <Button
            ...
            onClick={() => onEdit(id)}
          >
            <EditIcon sx={{ fontSize: "24px", color: "#ecec00" }} />
          </Button>
        )}
